...
pub struct ClientSession {
    ...
    pub audio_buffer: Arc<crate::audio::AudioRingBuffer>,
...
mod tests {
    ...
    fn test_client_session_lifecycle() {
        ...
        session.audio_buffer.push(0.5);
        assert_eq!(session.audio_buffer.len(), 1);

        session.reset();
        assert_eq!(session.get_client_id(), None);
        assert_eq!(session.audio_buffer.len(), 0);
    }
    ...
    fn test_multiple_independent_sessions() {
        ...
        session1.audio_buffer.push(1.0);
        session2.audio_buffer.push(2.0);
        session2.audio_buffer.push(3.0);

        assert_eq!(session1.audio_buffer.len(), 1);
        assert_eq!(session2.audio_buffer.len(), 2);

        session1.reset();
        assert_eq!(session1.get_client_id(), None);
        assert_eq!(session1.audio_buffer.len(), 0);
